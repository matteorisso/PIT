# -*- coding: utf-8 -*-
"""
Created on Tue Dec  1 18:48:18 2020
...
def data_generator(variant, seq_len):
    if os.path.exists(variant + "/corpus"):
        corpus = pickle.load(open(variant + '/corpus', 'rb'))
    else:
        print("Creating Corpus...")
        corpus = Corpus(variant + "/lambada_vocabulary_sorted.txt", variant)
        pickle.dump(corpus, open(variant + '/corpus', 'wb'))

    eval_batch_size = 1
    train_data = batchify(corpus.train, variant)
...
class Corpus(object):
    def __init__(self, dict_path, path):
        ...
        self.train = np.ndarray(self.tokenize(os.path.join(path, 'train-novels')))
